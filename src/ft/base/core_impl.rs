...
impl FungibleToken {
    pub fn new<S, V, V2>(prefix: S, prefix_vote: V, prefix_voted: V2) -> Self
    where
        S: IntoStorageKey, V: IntoStorageKey, V2: IntoStorageKey
    {
        let mut this = Self {
          accounts: LookupMap::new(prefix),
          total_supply: 0,
          account_storage_usage: 0,
          vote_by_account: LookupMap::new(prefix_vote),
          voted_by_account: LookupMap::new(prefix_voted),
        };
  ...
  pub(crate) fn assert_owner(&self) {
    ...
    pub fn internal_deposit(&mut self, account_id: &AccountId, amount: Balance) {
        ...
        if let Some(new_balance) = balance.checked_add(amount) {
          if new_balance > AMOUNT_MAX {
            env::panic_str(&format!("Maximum balance is {}", AMOUNT_MAX.to_string()));
          }
